...
/** A `ZPool2[E, A]` is a pool of items of type `A`, each of which may be associated with the acquisition and release of
  * resources. An attempt to get an item `A` from a pool may fail with an error of type `E`.
  */
trait ZPoolWithoutDisconnect[+Err, Item] {

  /** Retrieves an item from the pool in a scoped effect. Note that if acquisition fails, then the returned effect will
    * fail for that same reason. Retrying a failed acquisition attempt will repeat the acquisition attempt.
    */
  def get(implicit trace: Trace): ZIO[Scope, Err, Item]

  /** Invalidates the specified item. This will cause the pool to eventually reallocate the item, although this
    * reallocation may occur lazily rather than eagerly.
    */
  def invalidate(item: Item)(implicit trace: Trace): UIO[Unit]
}
object ZPoolWithoutDisconnect {

  /** Creates a pool from a fixed number of pre-allocated items. This method should only be used when there is no
    * cleanup or release operation associated with items in the pool. If cleanup or release is required, then the `make`
    * constructor should be used instead.
    */
  def fromIterable[A](iterable: => Iterable[A])(implicit trace: Trace): ZIO[Scope, Nothing, ZPoolWithoutDisconnect[Nothing, A]] =
    for {
      ...
      pool <- ZPoolWithoutDisconnect.make(get, iterable.size)
    } yield pool

  /** Makes a new pool of the specified fixed size. The pool is returned in a `Scope`, which governs the lifetime of the
    * pool. When the pool is shutdown because the `Scope` is closed, the individual items allocated by the pool will be
    * released in some unspecified order.
    */
  def make[R, E, A](get: => ZIO[R, E, A], size: => Int)(implicit
      trace: Trace
  ): ZIO[R with Scope, Nothing, ZPoolWithoutDisconnect[E, A]] =
    for {
      size <- ZIO.succeed(size)
      pool <- makeWith(get, size to size)(Strategy.None)
    } yield pool

  /** Makes a new pool with the specified minimum and maximum sizes and time to live before a pool whose excess items
    * are not being used will be shrunk down to the minimum size. The pool is returned in a `Scope`, which governs the
    * lifetime of the pool. When the pool is shutdown because the `Scope` is used, the individual items allocated by the
    * pool will be released in some unspecified order.
    * {{{
    * ZIO.scoped {
    *   ZPool2.make(acquireDbConnection, 10 to 20, 60.seconds).flatMap { pool =>
    *     ZIO.scoped {
    *       pool.get.flatMap {
  ...
  /** A more powerful variant of `make` that allows specifying a `Strategy` that describes how a pool whose excess items
    * are not being used will be shrunk down to the minimum size.
    */
  private def makeWith[R, R1, E, A](get: => ZIO[R, E, A], range: => Range)(strategy: => Strategy[R1, E, A])(implicit
      trace: Trace
  ): ZIO[R with R1 with Scope, Nothing, ZPoolWithoutDisconnect[E, A]] =
  ...
  private case class DefaultPool[R, E, A, S](
  ...
  ) extends ZPoolWithoutDisconnect[E, A] {
    ...
    /** Gets items from the pool and shuts them down as long as there are items free, signalling shutdown of the pool if
      * the pool is empty.
      */
  ...
  /** A `Strategy` describes the protocol for how a pool whose excess items are not being used should shrink down to the
    * minimum pool size.
    */
  private trait Strategy[-Environment, -Err, -Item] {
    ...
    /** Describes how the state of the strategy should be updated when an item is added to the pool or returned to the
      * pool.
      */
    def track(state: State)(item: Exit[Err, Item])(implicit trace: Trace): UIO[Unit]
  ...
  private object Strategy {
    ...
    /** A strategy that does nothing to shrink excess items. This is useful when the minimum size of the pool is equal
      * to its maximum size and so there is nothing to do.
      */
    ...
    /** A strategy that shrinks the pool down to its minimum size if items in the pool have not been used for the
      * specified duration.
      */
